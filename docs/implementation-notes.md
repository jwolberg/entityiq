...
## 2026-09-24 — Cloud Run deploy (ticket 0054, ADR-0005)

- **Owner decisions:**
  - a new `entityiq-demo` project;
  - private access (IAM plus `gcloud run services proxy`);
  - no Celery worker (the pipeline runs inline, in eager mode);
  - no Memorystore (in-memory sessions, max 1 instance).
- **One service, one image:** the root `Dockerfile` builds the UI and serves it
  from the API process (`app/ui.py`, turned on by `ENTITYIQ_UI_DIST`). The API
  answers under `/api`, like the Vite dev proxy. Local dev and compose are
  unchanged.
- **Tradeoff:** a KYB submission runs inside the HTTP request. Budgets are cut
  to 60 s per stage and 600 s per run, to fit the 900 s request timeout.
- **Worker pools** reached GA in April 2026, but the local gcloud can't load the
  command (its Python is missing `grpc`). Not needed with no worker.
- **Secret handling:** the DB password lives only inside the `DATABASE_URL`
  secret. The screening master key is created once and never regenerated by
  the script.

### 2026-09-24 — Deploy paused (ticket 0054): where to resume

The owner paused the deploy partway through `infra`.

Created in GCP:
- project `entityiq-demo`, linked to the Side Projects billing account;
- the Run, SQL Admin, Artifact Registry, Cloud Build and Secret Manager APIs;
- Artifact Registry repo `entityiq` (us-central1);
- Cloud SQL `entityiq-pg` (Postgres 16, db-f1-micro). The create was already
  issued and continues inside GCP (it showed `PENDING_CREATE`). It bills while
  it runs, at about $10/month.

Not created yet:
- the `entityiq` database and user;
- the `entityiq-database-url` and `entityiq-screening-master-key` secrets;
- the `entityiq-run` service account and its roles;
- the image, the migrate and seed jobs, and the service.

To resume, run the whole thing. Each step checks before it creates, so this
picks up where it stopped:

    PROJECT_ID=entityiq-demo BILLING_ACCOUNT=0033B7-42DFC1-DAEC05 ./scripts/deploy-cloud-run.sh all

Gotchas:
- Right after the APIs are enabled, the first Artifact Registry call can
  return PERMISSION_DENIED for a minute (IAM propagation). Wait and re-run.
- Run `build`, `migrate` and `service` from the same commit, because they
  share the image tag.
- Once the service is up, set `last-verified` in
